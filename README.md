...
| COL 1–4 | P0.28, P0.11, P0.31, P0.30 |
| COL 5 | P1.05 |
...
- `to_id` — destination device ID (0 = broadcast, accepted by all)
...
Press **A + B simultaneously** to cycle through device IDs (0–5). The current target ID is shown as a digit on the LED matrix. Repeat until the desired ID is displayed.
...
Digits 0–9 are also supported (e.g. 0 = `−−−−−`, 1 = `·−−−−`).
...
```c
#define My_ID           1       // This device's ID (change per device: 1–5)
...
To deploy to a new device, change only `My_ID`. All other settings must be identical across all devices on the network.
